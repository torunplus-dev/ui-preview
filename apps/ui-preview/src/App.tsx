...
function AppInner() {
  ...
  const sampleControlRows = [
    { key: 'control-1', id: 'CTRL-001', label: 'ユーザー名入力欄', status: '有効' },
    { key: 'control-2', id: 'CTRL-002', label: 'ステータス絞り込み', status: '有効' },
    { key: 'control-3', id: 'CTRL-003', label: '一括更新ボタン', status: '無効' }
  ];
      ...
      <Layout style={{ minHeight: 0, overflow: 'hidden' }}>
        ...
        <Layout style={{ minHeight: 0, overflow: 'hidden' }}>
          ...
          <Content style={{ padding: 16, overflow: 'auto', minHeight: 0 }}>
            ...
            <Card
              size="small"
              title="サンプルテーブルコントロール"
              style={{ marginBottom: 16 }}
              extra={<Typography.Text type="secondary">追加UI</Typography.Text>}
            >
              <Table
                size="small"
                columns={sampleControlColumns}
                dataSource={sampleControlRows}
                pagination={false}
              />
            </Card>
            <Tabs
